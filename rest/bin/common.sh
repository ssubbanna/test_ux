...
#  saas_ux_tag      - the MAJOR.MINOR.PATCH version of saas_ux installed in the image
#  short_tag    - MAJOR.MINOR from ${saas_ux_tag}
#  latest_short - contains the highest version beginning with ${short_tag}
#  tagged_build - true if build was tagged, else false
#  tag          - tag image with this value (if tagged_build saas_ux_tag else latest)
...
if [[ ${CIRCLE_TAG} =~ ^v(.+)$ ]]; then
  ...
  # Build and tag an image using the highest saas_ux release
  tagged_build=false
  tag='latest'
  saas_ux_tag=${latest}
fi
